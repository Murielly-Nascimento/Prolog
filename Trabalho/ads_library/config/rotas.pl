...
:-use_module(frontend(pagina_login),[]).

:- http_handler( root(login), pagina_login:login, [ id(login) ]).

%% A página de cadastro de novos usuários

:-use_module(frontend(pagina_cadastro),[]).

:- http_handler( root(cadastro), pagina_cadastro:cadastro, []).


%% Páginas de usuários

:-use_module(frontend(pagina_administrador),[]).

:- http_handler(root(admin/Id), pagina_administrador:entrada(Id),
                []).
...
:- http_handler( api1(funcionarios/Id), funcionarios(Método, Id),
                [ method(Método),
                  methods([ get, post, put, delete ]) ]).
